...
async def create_arg_string(num):
    L = []
    for i in range(num):
        L.append('?')
...
class ModelMetaclass(type):
    def __new__(mcs, name, bases, attrs):
        ...
        attrs['__insert__'] = 'insert into %s (%s, `%s`) values (%s)' % (tableName, ','.join(escaped_fields), primary_key, create_arg_string(len(escaped_fields) + 1))
        attrs['__update__'] = 'update %s set %s where `%s`= ?' % (tableName, ','.join(map(lambda f: '`%s` = ?' % (mapping.get(k).name or f)), fields), primaryKey)
